...
public class GUI_Run extends JFrame implements KeyListener, ActionListener{
	...
	private SerializeGame sg = new SerializeGame();
	private DeserializeGame dg = new DeserializeGame();
    ...
    public GUI_Run(int color1, int color2, int color3) {
		...
		saveGameBtn.setBounds(825,710,400,60);
		loadGameBtn.setFont(f);
		loadGameBtn.setBounds(825, 630, 400, 60);
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getActionCommand().equals("Save Game")) {
			sg.saveGame(aSnake, bSnake);
			sg.saveGamePoint(p1, p2, p3);
			System.out.println("saving");
		}
		else if (e.getActionCommand().equals("Load Game")) {
			aSnake = dg.loadGameA(aSnake);
			bSnake = dg.loadGameB(bSnake);
			p1 = dg.loadPoint1(p1);
			p2 = dg.loadPoint1(p2);
			p3 = dg.loadPoint1(p3);
    ...
    public static void main(String[] args) {
    	javax.swing.SwingUtilities.invokeLater(new Runnable() {
    		public void run() {
				...
				GUI_Run faceWindow = new GUI_Run(color1, color2, color3);
