...
public class DrawingApp extends JFrame implements KeyListener {
	...
	public Shape objectShapeF(int num) {
		Shape a = foods.get(num);
		return a;
	}
	...
	Shape foodA = objectShapeF(0);
	/*Point pointFood = foodA.getTopLeft();
	Shape snakeA = snakes.get(0);
	Point pointSnake = snakeA.getTopLeft();

	public boolean checkFood(){
		boolean flag = true;
		if (pointSnake.getXCoord() == pointFood.getXCoord() && pointFood.getYCoord() == pointSnake.getYCoord()){
			flag = false;
		}
		return flag;
	}
	...
	public void generateFood(){
	  while (checkFood() == false){
		...
		Point newFood = new Point(randX.nextInt(WINDOW_WIDTH), randY.nextInt(WINDOW_HEIGHT));
		Shape food = new Square(newFood, DEFAULT_FOOD_SIZE);
		foods.add(food);
	  }
	}*/
